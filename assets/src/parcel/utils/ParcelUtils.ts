...
import { ParcelType } from '../ParcelEnum';

export class ParcelUtils {
    public toKey(x: number, y: number): string {
        return `${x},${y}`;
    }

    public pickWeightedParcelType(weights: Record<ParcelType, number>): ParcelType {
    ...
    public pickParcelSpritePath(spritePaths: Record<ParcelType, string[]>, type: ParcelType): string {
    ...
    public assignParcelAt(
        ...
        const nextType = type ?? this.pickWeightedParcelType(weights);
        const boardCell = board[y][x];
        boardCell.parcelType = nextType;
        boardCell.parcelSpritePath = this.pickParcelSpritePath(spritePaths, nextType);
    }

    public rerollBoardParcels(
        board: BoardCell[][],
        weights: Record<ParcelType, number>,
        spritePaths: Record<ParcelType, string[]>,
    ): void {
        for (let y = 0; y < BOARD_ROWS; y++) {
            for (let x = 0; x < BOARD_COLS; x++) {
                this.assignParcelAt(board, x, y, weights, spritePaths);
            }
        }
    }

    public fillBoardParcels(
        board: BoardCell[][],
        type: ParcelType,
        weights: Record<ParcelType, number>,
        spritePaths: Record<ParcelType, string[]>,
        ...
        for (let y = 0; y < BOARD_ROWS; y++) {
            for (let x = 0; x < BOARD_COLS; x++) {
                this.assignParcelAt(board, x, y, weights, spritePaths, type);
            }
        }
    }
}
