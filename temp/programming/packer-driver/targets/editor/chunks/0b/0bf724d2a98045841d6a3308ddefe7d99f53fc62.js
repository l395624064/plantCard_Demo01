    ...
    execute: function () {
      ...
      _export("GameView", GameView = class GameView {
        ...
        render(state) {
          ...
          const statusText = state.status === 'playing' ? state.preview ? state.preview.reason : state.message : `${state.message}`;
        ...
        applyPlantSpriteFitFromNode(sprite) {
          const fit = sprite.node.__plantFit;
        ...
        createTopInfo() {
          ...
          const timeWheelYearLabel = this.createLabel(timeWheelCenterNode, 'TimeWheelYearLabel', wheelCenterSize * 1.55, 32, 0, 18, clamp(this.rootWidth * 0.032, 20, 28), HorizontalTextAlignment.CENTER, VerticalTextAlignment.CENTER);
          ...
          const progressLabel = this.createLabel(panel, 'ScoreProgressLabel', progressWidth - 18, 26, progressCenterX, progressY, clamp(this.rootWidth * 0.026, 15, 18), HorizontalTextAlignment.CENTER, VerticalTextAlignment.CENTER);
          ...
          const statsLabel = this.createLabel(panel, 'InfoStatsLabel', statsWidth, lowerHeight, statsX, lowerCenterY, clamp(this.rootWidth * 0.028, 16, 20), HorizontalTextAlignment.LEFT, VerticalTextAlignment.CENTER);
          const reserveLabel = this.createLabel(panel, 'ReserveLabel', reserveWidth - 18, lowerHeight - 12, reserveX, lowerCenterY, clamp(this.rootWidth * 0.024, 13, 16), HorizontalTextAlignment.LEFT, VerticalTextAlignment.CENTER);
          reserveLabel.color = makeColor(132, 118, 96, 255);
          const reserveNode = this.makeNode('ReservePanel', reserveWidth, lowerHeight, reserveX, lowerCenterY);
          ...
          const gmButton = this.makePanel('GmButton', gmWidth, 30, gmX, progressY, makeColor(229, 235, 248, 255));
        ...
        createMessage() {
          ...
          panel.active = false;
          this.root.addChild(panel);
          const label = this.createLabel(panel, 'MessageLabel', 1, 1, 0, 0, 1, HorizontalTextAlignment.LEFT, VerticalTextAlignment.CENTER);
        ...
        createBoard() {
          ...
          const cellSize = this.boardSize / (_crd && BOARD_COLS === void 0 ? (_reportPossibleCrUseOfBOARD_COLS({
            error: Error()
          ...
          for (let y = 0; y < (_crd && BOARD_ROWS === void 0 ? (_reportPossibleCrUseOfBOARD_ROWS({
            ...
            for (let x = 0; x < (_crd && BOARD_COLS === void 0 ? (_reportPossibleCrUseOfBOARD_COLS({
              ...
              const label = this.createLabel(board, `CellLabel-${x}-${y}`, cellSize, cellSize, -this.boardSize / 2 + cellSize * (x + 0.5), this.boardSize / 2 - cellSize * (y + 0.5), 20, HorizontalTextAlignment.CENTER, VerticalTextAlignment.CENTER);
              row.push(label);
              const levelLabel = this.createLabel(board, `CellLevel-${x}-${y}`, cellSize, 22, -this.boardSize / 2 + cellSize * (x + 0.5), this.boardSize / 2 - cellSize * y - cellSize * 0.835, 13, HorizontalTextAlignment.CENTER, VerticalTextAlignment.CENTER);
        ...
        ensureHandViewCount(count) {
          ...
          while (this.handViews.length < count) {
            ...
            for (let y = 0; y < (_crd && CARD_TEMPLATE_ROWS === void 0 ? (_reportPossibleCrUseOfCARD_TEMPLATE_ROWS({
              error: Error()
            }), CARD_TEMPLATE_ROWS) : CARD_TEMPLATE_ROWS); y++) {
              for (let x = 0; x < (_crd && CARD_TEMPLATE_COLS === void 0 ? (_reportPossibleCrUseOfCARD_TEMPLATE_COLS({
                error: Error()
            ...
            const costLabel = this.createLabel(node, `Cost-${i}`, 44, 44, -this.handCardWidth / 2 + 28, this.handCardHeight / 2 - 30, clamp(this.rootWidth * 0.032, 18, 22), HorizontalTextAlignment.CENTER, VerticalTextAlignment.CENTER);
            costLabel.string = '0';
            const title = this.createLabel(node, `Title-${i}`, this.handCardWidth - 24, 30, 0, this.handCardHeight * 0.08, clamp(this.rootWidth * 0.03, 17, 21), HorizontalTextAlignment.CENTER, VerticalTextAlignment.CENTER);
            const descLabel = this.createLabel(node, `Desc-${i}`, this.handCardWidth - 20, 52, 0, -this.handCardHeight * 0.22, clamp(this.rootWidth * 0.024, 13, 17), HorizontalTextAlignment.CENTER, VerticalTextAlignment.TOP);
            descLabel.overflow = Label.Overflow.CLAMP;
            const typeLabel = this.createLabel(node, `Type-${i}`, this.handCardWidth - 20, 22, 0, -this.handCardHeight / 2 + 18, clamp(this.rootWidth * 0.022, 12, 15), HorizontalTextAlignment.CENTER, VerticalTextAlignment.CENTER);
            node.on(Node.EventType.TOUCH_START, event => this.onHandCardTouchStart(i, event), this);
            node.on(Node.EventType.MOUSE_DOWN, event => this.onHandCardMouseDown(i, event), this);
        ...
        createPreplaceButton(parent, title, x, y, width, onClick) {
          const h = 44;
          const fillColor = title === '放置' ? makeColor(214, 242, 198, 255) : title === '取消' ? makeColor(250, 216, 201, 255) : makeColor(244, 226, 166, 255);
          const strokeColor = title === '放置' ? makeColor(76, 138, 64, 255) : title === '取消' ? makeColor(176, 91, 72, 255) : makeColor(160, 120, 50, 255);
          ...
          const applyButtonVisual = (scale, liftY) => {
        ...
        isPointerNearLockedPreview(boardTransform, uiX, uiY) {
          ...
          const preview = (_this$lastRenderState = this.lastRenderState) == null ? void 0 : _this$lastRenderState.preview;
        ...
        getActivePlacementBounds() {
          ...
          const slot = (_ref = (_this$lastRenderState3 = (_this$lastRenderState4 = this.lastRenderState) == null ? void 0 : _this$lastRenderState4.handSlots.find(item => item.selected)) != null ? _this$lastRenderState3 : this.preplaceHandIndex !== null ? (_this$lastRenderState5 = this.lastRenderState) == null ? void 0 : _this$lastRenderState5.handSlots[this.preplaceHandIndex] : null) != null ? _ref : null;
          const card = (_slot$card = slot == null ? void 0 : slot.card) != null ? _slot$card : null;
        ...
        tryBeginPreplace(index, pointerYUi) {
          ...
          const card = slot == null ? void 0 : slot.card;
        ...
        cancelPreplaceFlow(notify = true) {
        ...
        syncPreplaceBar(state) {
          ...
          const scale = state.preplaceLocked ? this.preplace2BoardDragging ? 0.94 : 1 : 1;
        ...
        refreshArcAndBarPosition(state) {
          ...
          const idealY = useTopStack ? center.y + clamp(this.boardSize * 0.22, 88, 120) : center.y + clamp(this.boardSize * 0.035, 10, 18);
          const x = clamp(idealX, -this.rootWidth / 2 + barTransform.contentSize.width / 2 + margin, this.rootWidth / 2 - barTransform.contentSize.width / 2 - margin);
          const y = clamp(idealY, -this.rootHeight / 2 + barTransform.contentSize.height / 2 + margin, this.rootHeight / 2 - barTransform.contentSize.height / 2 - margin);
        ...
        renderHand(handSlots) {
          ...
          for (let i = 0; i < this.handViews.length; i++) {
            ...
            const isSelected = !!(slot != null && slot.selected) || inPreplace;
            ...
            const gap = 6;
            const cellW = (artW - gap * ((_crd && CARD_TEMPLATE_COLS === void 0 ? (_reportPossibleCrUseOfCARD_TEMPLATE_COLS({
              error: Error()
            ...
            const cellH = (artH - gap * ((_crd && CARD_TEMPLATE_ROWS === void 0 ? (_reportPossibleCrUseOfCARD_TEMPLATE_ROWS({
              error: Error()
            ...
            for (const cell of card.cells) {
              ...
              const x = originX + cell.x * (cellW + gap); // 手牌上的格子朝向要与棋盘放置预览一致：cell.y 越大，视觉上越靠下。

              const y = artTopY - cell.y * (cellH + gap) - cellH;
              const artIndex = cell.y * (_crd && CARD_TEMPLATE_COLS === void 0 ? (_reportPossibleCrUseOfCARD_TEMPLATE_COLS({
                error: Error()
              ...
              (_artCell$node$getComp = artCell.node.getComponent(UITransform)) == null || _artCell$node$getComp.setContentSize(cellW, cellH);
        ...
        getFanSlotTransform(index, count) {
          ...
          const spreadX = count <= 1 ? 0 : Math.min(clamp(this.handCardWidth * 0.48, 62, 92), (this.panelWidth - this.handCardWidth * 1.1) / Math.max(count - 1, 1));
        ...
        applyHandFanLayout(activeCount = ((_this$lastRenderState10, _this$lastRenderState11) => (_this$lastRenderState10 = (_this$lastRenderState11 = this.lastRenderState) == null ? void 0 : _this$lastRenderState11.handSlots.length) != null ? _this$lastRenderState10 : 0)()) {
        ...
        reorderHandSiblingsForHover(activeCount = ((_this$lastRenderState13, _this$lastRenderState14) => (_this$lastRenderState13 = (_this$lastRenderState14 = this.lastRenderState) == null ? void 0 : _this$lastRenderState14.handSlots.length) != null ? _this$lastRenderState13 : 0)()) {
          if (!this.handPanel) {
            return;
          }
        ...
        syncPreviewRotateTween(state) {
          ...
          if (this.previewRotateLogical !== state.rotation) {
            ...
            const prevPreview = (_this$lastRenderState15 = (_this$lastRenderState16 = this.lastRenderState) == null ? void 0 : _this$lastRenderState16.preview) != null ? _this$lastRenderState15 : null;
            this.previewRotateLogical = state.rotation;
            Tween.stopAllByTarget(this.previewRotateTweenState);
        ...
        tracePreviewRect(graphics, x, y, width, height, pivotX, pivotY, angleRad) {
          ...
          const corners = [this.rotatePointAroundPivot(x, y, pivotX, pivotY, angleRad), this.rotatePointAroundPivot(x + width, y, pivotX, pivotY, angleRad), this.rotatePointAroundPivot(x + width, y + height, pivotX, pivotY, angleRad), this.rotatePointAroundPivot(x, y + height, pivotX, pivotY, angleRad)];
        ...
        startHandRefillAnimation(removedIdx, snap, oldIds, handSlots) {
          ...
          for (let i = 0; i < handSlots.length; i++) {
            ...
            node.setScale(1, 1, 1);
            node.active = true;
            const card = (_slot$card2 = slot == null ? void 0 : slot.card) != null ? _slot$card2 : null;

            if (!card) {
              var _snap$positions$i;

              const base = (_snap$positions$i = snap.positions[i]) != null ? _snap$positions$i : targetPos;
        ...
        syncBoardPlants(state, cellSize) {
          ...
          for (let y = 0; y < (_crd && BOARD_ROWS === void 0 ? (_reportPossibleCrUseOfBOARD_ROWS({
            ...
            for (let x = 0; x < (_crd && BOARD_COLS === void 0 ? (_reportPossibleCrUseOfBOARD_COLS({
              ...
              const view = this.boardPlantViews[y * (_crd && BOARD_COLS === void 0 ? (_reportPossibleCrUseOfBOARD_COLS({
                error: Error()
              }), BOARD_COLS) : BOARD_COLS) + x];
          ...
          if (state.preview) {
            ...
            for (const cell of state.preview.cells) {
              var _view$node$getCompone2;

              if (previewIndex >= this.previewPlantViews.length || cell.blocked || cell.overlapsSame || cell.overlapsDiff || cell.plantVariant === null) {
                continue;
              }
              ...
              view.node.active = true;
              view.node.setPosition(centerX, centerY, 0);
              (_view$node$getCompone2 = view.node.getComponent(UITransform)) == null || _view$node$getCompone2.setContentSize(cellSize, cellSize);
              view.graphics.clear();
          ...
          if (state.preview) {
            ...
            for (const cell of preview.cells) {
              ...
              if (!cell.blocked && cell.overlapsSame) {
                ...
                if (cell.overlapsSame || cell.overlapsDiff) {// handled by explicit mark rendering above
                }
              } else {
                graphics.lineWidth = 2;

                if (cell.overlapsSame || cell.overlapsDiff) {// overlap state now uses gray badge + mark only
                }
              }
            }
        ...
        resolveAnchor(boardTransform, uiX, uiY) {
          ...
          const x = clamp(pointerCellX - (activeBounds.width - 1), 0, (_crd && BOARD_COLS === void 0 ? (_reportPossibleCrUseOfBOARD_COLS({
            error: Error()
          }), BOARD_COLS) : BOARD_COLS) - activeBounds.width);
          const y = clamp(pointerCellY - (activeBounds.height - 1), 0, (_crd && BOARD_ROWS === void 0 ? (_reportPossibleCrUseOfBOARD_ROWS({
            error: Error()
        ...
        measureLayout() {
          ...
          const rootTransform = this.root.getComponent(UITransform);
          const contentSize = rootTransform == null ? void 0 : rootTransform.contentSize;
