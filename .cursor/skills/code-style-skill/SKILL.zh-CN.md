...
- 数据结构可参考项目内现有 `tmpTrace` 字段设计（如 `core/tmpTrace/TmpTraceEnum` 的语义），并允许按项目实际做等价扩展。
- “AI理解优先原则”最小验收标准：
  - 能还原执行链（步骤顺序与上下游关系）；
  - 能进行预期/观察对比；
  - 能定位异常因果（关键决策与原因可追踪）。
...
- 交互式验收清单面板（类网页 Excel，待讨论）：
  ...
  - 用户勾选行为可被 AI 感知；
  - AI 可通过 `browsermcp`（浏览器环境）或 `tmp/*` 日志文件（非浏览器环境）自动对齐清单状态。
- 浏览器日志落盘桥接（待实现）：
  - 通过本地桥接进程将浏览器 trace 近实时写入 `tmpTrace/tmp/*.jsonl`。
- IDE 终端自动轮询日志进程（待实现）：
  - 在 `browsermcp` 连通后自动轮询并打印日志，不占用 agent 聊天窗口。
