...
- 维护一份“辅助引导推荐 skills 清单”（以下简称 `skills 清单`）。
- 触发时机（任一命中即触发）：
  - `code-style-skill` 首次安装/加载完成后；
  - 用户开启“辅助引导”时。
- 助手在触发后必须执行：
  - 检查 `skills 清单` 各项是否已安装；
  - 输出缺失项清单（若有）；
  - 询问用户是否需要一键安装缺失项。
- 若用户同意安装，助手应按“工具链优先级总则”自动执行安装与最小可用性验证，并输出安装回执。
- 若用户拒绝安装，助手应继续当前任务，不得阻断开发流程。

### [项目示例 - 非强制] 辅助引导推荐 skills 清单
...
- `code-style-skill` 必须维护远程更新源地址，用于在其它项目复用时进行版本同步。
- 助手在其它项目启用本 skill 时，应先检查远程版本，再决定是否执行本地更新。
- 最小远程检查项：
  - 远程可访问性检查，
  - 关键文件完整性检查（`SKILL.md`、`SKILL.zh-CN.md`、`SKILL.history*`、`SKILL.pending*`），
  - 本地与远程版本号对比。
- 若远程有更新，助手必须回执：
  - 计划更新文件清单，
  - 版本变化，
